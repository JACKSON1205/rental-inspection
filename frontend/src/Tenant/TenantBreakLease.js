...
export default function TenantBreakLeasePage (props) {
  ...
  const [vacateDate, setVacateDate] = React.useState(new Date());
  const formattedDate = `${vacateDate.getFullYear()}-${vacateDate.getMonth() + 1}-${vacateDate.getDate()}`;
    ...
    <div className={classes.root}>
      ...
      <main className={classes.content}>
        ...
        <Container maxWidth="lg" className={classes.container}>
          ...
          <Typography component="h1" variant="h5" className={classes.choose} align="center">
            Choose the date you want to vacate (you may have to pay a break lease fee):
          </Typography>
          <div>
            <DayPicker
              selectedDays={vacateDate}
              onDayClick={(date) => { setVacateDate(date) }}
          ...
          <div className={classes.buttonsContainer}>
            <Button
              ...
              onClick={(event) => { event.preventDefault(); noticesBreakLeaseProperty(formattedDate, propertyId); }}
            >
              Submit
            </Button>
            <Button
...
async function noticesBreakLeaseProperty (vacateDate, propertyId) {
  const artifactJSON = {
    leave_date: vacateDate,
  ...
  if (res.status === 200 || res.status === 201) {
    console.log('Notices break lease property success');
    sessionStorage.setItem('token', data.token);
    window.location = '/tenant/request';
    alert('Notices break lease property success');
  } else if (res.status === 400 || res.status === 403) {
    console.log('Notices break lease property fail');
    console.log(data.error);
    alert(res.status + ': ' + data.error);
  } else {
    console.log('Notices break lease property fail');
    alert('Notices break lease property fail');
  }
  sessionStorage.setItem('token', data.token);
}

TenantBreakLeasePage.propTypes = {
