      ...
      <main className={classes.content}>
        ...
        <Container maxWidth="lg" className={classes.container}>
          ...
          <Button
            ...
            variant="contained"
            color="secondary"
            id="tenantAddContactCancelButton"
          ...
          <Button
            ...
            variant="contained"
            color="primary"
            id="tenantAddContactButton"
            className={classes.saveButton}
            onClick={(event) => { event.preventDefault(); saveNewContact(email); }}
          >
            Save
          </Button>
          {/* <GetAllContactInfo /> */}
...
async function saveNewContact (email) {
  const options = {
    method: 'POST',
    headers: {
      accept: 'application/json',
  ...
  if (res.status === 200 || res.status === 201) {
    ...
    sessionStorage.setItem('token', data.token);
    alert('Successfully sending connection request to manager');
