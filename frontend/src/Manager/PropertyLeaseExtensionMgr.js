...
export default function propertyDetailPage (props) {
  ...
  const [extensionDate, setExtensionDate] = React.useState(new Date());
  const formattedDate = `${extensionDate.getFullYear()}-${extensionDate.getMonth() + 1}-${extensionDate.getDate()}`;
      ...
      <main className={classes.content}>
        ...
        <Container maxWidth="lg" className={classes.container}>
          ...
          <Typography variant="h4" component="h2" align="center">
            Please select the lease extension date
          </Typography>
          <br />
          <DayPicker
            selectedDays={extensionDate}
            onDayClick={(date) => { setExtensionDate(date) }}
          ...
          <div className={classes.buttonsContainer}>
            <Button
              ...
              onClick={(event) => { event.preventDefault(); extendLease(propertyId, formattedDate); }}
            >
              Save
            </Button>
            <Button
...
// Make api extending lease of the property
async function extendLease (propertyId, expirationDate) {
  ...
  if (res.status === 200 || res.status === 201) {
    console.log('Extend Lease success');
    sessionStorage.setItem('token', data.token);
    alert('Extend Lease success');
