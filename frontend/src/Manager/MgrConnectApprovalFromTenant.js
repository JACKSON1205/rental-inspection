...
export default function NotificationDetail (props) {
  ...
  React.useEffect(() => {
    ...
    async function getClientName () {
      ...
      if (res.status === 200) {
        setClientName(data.artifact.artifact_json.name);
        sessionStorage.setItem('token', data.token);
      } else if (res.status === 401) {
        console.log('unauthorized: redirecting to login');
        console.log(data.error)
      ...
      <main className={classes.content}>
        ...
        <Container maxWidth="lg" className={classes.container}>
          ...
          <Typography component="h1" variant="h4" align="center">
            Do you accept the connection with {clientName} ?
          </Typography>
          <br />
          <br />
          <Button
            ...
            onClick={(event) => { event.preventDefault(); acceptConnection(artifactId[notificationIndex], clientId[notificationIndex]); }}
          >
            Accept
          </Button>
          <Button
            ...
            onClick={(event) => { event.preventDefault(); declineConnection(artifactId[notificationIndex], clientId[notificationIndex]); }}
          >
            Decline
          </Button>
          <Box pt={4}>
...
async function acceptConnection (artifactId, clientId) {
  const options = {
    method: 'PATCH',
    headers: {
      accept: 'application/json',
  ...
  if (res.status === 200 || res.status === 201) {
    ...
    sessionStorage.setItem('token', data.token);
    alert('Successfully accept connection');
...
async function declineConnection (artifactId, clientId) {
  const options = {
    method: 'PATCH',
    headers: {
      accept: 'application/json',
  ...
  if (res.status === 200 || res.status === 201) {
    ...
    sessionStorage.setItem('token', data.token);
    alert('Successfully denied connection');
    window.location = '/manager/notification';
  } else if (res.status === 400 || res.status === 403) {
    console.log(data.error);
    alert(res.status + ': ' + data.error);
