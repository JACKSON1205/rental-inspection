...
export default function ManagerBreakLeaseNotice (props) {
  ...
  React.useEffect(() => {
    ...
    async function getPropertyId () {
      ...
      if (res.status === 200) {
        ...
        setVacateDate(data.artifact.artifact_json.leave_date);
      } else if (res.status === 401) {
        console.log('unauthorized: redirecting to login');
        console.log(data.error)
        alert(res.status + ': ' + data.error);
        ...
        <Container maxWidth="lg" className={classes.container}>
          ...
          <div className={classes.buttonsContainer}>
            <Button
              ...
              onClick={(event) => { event.preventDefault(); sendBreakLeaseNotice(ownerEmail, vacateDate, propertyDetail.property_id); }}
            >
              Submit
            </Button>
            <Button
...
async function sendBreakLeaseNotice (ownerEmail, vacateDate, propertyId) {
  const artifactJSON = {
    property_id: propertyId,
    break_lease_date: vacateDate,
  };
  const options = {
    method: 'POST',
    headers: {
  ...
  if (res.status === 200 || res.status === 201) {
    console.log('Send break lease notice');
    sessionStorage.setItem('token', data.token);
    alert('Sending break lease notice success !!!');
    window.location = '/manager/notification';
  } else if (res.status === 400 || res.status === 403) {
    console.log(data.error);
    alert(res.status + ': ' + data.error);
  } else {
    console.log('Fail to send break lease notice');
    alert('Fail to send break lease notice');
  }
  sessionStorage.setItem('token', data.token);
}

ManagerBreakLeaseNotice.propTypes = {
