...
export default function ManagerLeaseExtensionFromTenant (props) {
      ...
      <main className={classes.content}>
        ...
        <Container maxWidth="lg" className={classes.container}>
          ...
          <Typography component="h1" variant="h4" align="center">
            Do you approve the property lease extension request from {propertyDetail.tenant_first_name} {propertyDetail.tenant_last_name}?
          </Typography>
          <br />
          <br />
          ...
          <div className={classes.buttonsContainer}>
            <Button
              ...
              onClick={(event) => { event.preventDefault(); forwardToOwner(ownerEmail, extensionDate, propertyDetail.property_id, clientId[notificationIndex], artifactId[notificationIndex]); }}
            >
              Forward to Owner
            </Button>
          </div>
...
// Make api forwarding lease extension detail to tenant
async function forwardToOwner (ownerEmail, extensionDate, propertyId, tenantId, artifactId) {
  const artifactJSON = {
  ...
  if (res.status === 200 || res.status === 201) {
    console.log('Forward to owner success');
    sessionStorage.setItem('token', data.token);
    alert('Successfully forward lease extension detail to owner !!!');
    // window.location = '/manager/notification';
  } else if (res.status === 400 || res.status === 403) {
    console.log(data.error);
    alert(res.status + ': ' + data.error);
  } else {
    console.log('Fail to forward detail to owner');
    alert('Fail to forward detail to owner');
  }
  sessionStorage.setItem('token', data.token);
}

ManagerLeaseExtensionFromTenant.propTypes = {
