...
export default function Home () {
  ...
  const [seleteDateAndTime, setSeleteDateAndTime] = React.useState(new Date());
  console.log(seleteDateAndTime);
  const formattedDate = `${seleteDateAndTime.getFullYear()}-${seleteDateAndTime.getMonth() + 1}-${seleteDateAndTime.getDate()}`;
  const formattedTime = `${seleteDateAndTime.getHours()}:${seleteDateAndTime.getMinutes() + 1}:${seleteDateAndTime.getSeconds()}`;
  console.log(formattedDate);
  console.log(formattedTime);
  const [rolesSelection, setRolesSelection] = React.useState('');
      ...
      <main className={classes.content}>
        ...
        <Container maxWidth="lg" className={classes.container}>
          ...
          <FormControl fullWidth>
            <InputLabel id="noticeRoleSelectionLable">Role Selection</InputLabel>
            <Select
              labelId="noticeRoleSelectionLable"
              id="noticeRoleSelect"
              value={rolesSelection}
              label="Role Selection"
              onChange={(event) => { setRolesSelection(event.target.value) }}
            >
          ...
          <LocalizationProvider dateAdapter={AdapterDateFns} className={classes.time}>
            <Stack spacing={3}>
              <DateTimePicker
                value={seleteDateAndTime}
                onChange={setSeleteDateAndTime}
          ...
          <Button
            ...
            onClick={(event) => { event.preventDefault(); requestNotice(recipientEmail, rolesSelection, inputDescription, formattedDate, formattedTime); }}
          >
            Submit
          </Button>
          <Button
...
async function requestNotice (recipientEmail, rolesSelection, inputDescription, noticeDate, noticeTime) {
  let tenant = false;
  let owner = false;
  if (rolesSelection === 'Tenant') {
    tenant = true;
    owner = false;
  } else if (rolesSelection === 'Owner') {
    tenant = false;
    owner = true;
  }
  ...
  console.log(options);
  const res = await fetch(url + '/admin/schedule', options);
  const data = await res.json();
  // console.log(data);

  if (res.status === 200 || res.status === 201) {
    console.log('Successfully sending schedule notice');
    sessionStorage.setItem('token', data.token);
    window.location = '/manager/home/';
    alert('Successfully sending schedule notice');
    ...
    console.log('Fail to send schedule notice');
    alert('Fail to send schedule notice');
  }
  sessionStorage.setItem('token', data.token);
}
