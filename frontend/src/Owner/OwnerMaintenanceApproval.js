...
export default function NotificationDetail (props) {
  ...
  React.useEffect(() => {
    ...
    async function getArtifact () {
      ...
      if (res.status === 200) {
        setRepairDescription(data.artifact_resp.artifact_json.description);
        setRepairImage(data.artifact_resp.artifact_json.repairing_image);
        sessionStorage.setItem('token', data.token);
      ...
      <main className={classes.content}>
        ...
        <Container maxWidth="lg" className={classes.container}>
          ...
          <Typography component="h1" variant="h4" align="center">
            Do you approve this maintenance ?
          </Typography>
          <br />
          <br />
          ...
          <Typography component="h1" variant="h6" align="center">
            Maintenance Description: {repairDescription}
          </Typography>
          <br />
          <br />
          ...
          <div className={classes.buttonsContainer}>
            <Button
              ...
              variant="contained"
              color="primary"
              id="tenantAcceptConnectionButton"
              className={classes.acceptButton}
              onClick={(event) => { event.preventDefault(); approveMaintenance(artifactId[notificationIndex]); }}
            >
              Approve
            </Button>
            <Button
              ...
              onClick={(event) => { event.preventDefault(); declineMaintenance(artifactId[notificationIndex]); }}
            >
              Decline
            </Button>
          </div>
...
// Make api request of accepting connection from manager
async function approveMaintenance (artifactId) {
  const options = {
    method: 'PATCH',
    headers: {
      accept: 'application/json',
  ...
  const res = await fetch(url + '/admin/requests/' + artifactId, options);
  const data = await res.json();
  console.log(data);

  if (res.status === 200 || res.status === 201) {
    console.log('Approve Maintenance');
    sessionStorage.setItem('token', data.token);
    ...
    console.log('Fail to approve Maintenance');
    alert('Fail to approve Maintenance');
  }
  sessionStorage.setItem('token', data.token);
}

// Make api request of denieding connection from manager
async function declineMaintenance (artifactId) {
  const options = {
    method: 'PATCH',
    headers: {
      accept: 'application/json',
  ...
  if (res.status === 200 || res.status === 201) {
    console.log('Denied approval Success');
    sessionStorage.setItem('token', data.token);
    window.location = '/owner/notification';
  } else if (res.status === 400 || res.status === 403) {
    console.log(data.error);
    ...
    console.log('Fail to denied approval');
    alert('Fail to denied approval');
  }
  sessionStorage.setItem('token', data.token);
}
