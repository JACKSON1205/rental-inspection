...
export default function AddPropertyOwner () {
  ...
  const [leaseStatus, setLeaseStatus] = React.useState(false);
      ...
      <main className={classes.content}>
        ...
        <Container maxWidth="lg" className={classes.container}>
          ...
          <FormControl fullWidth>
            <InputLabel id="leaseStatusLabel">Lease Status</InputLabel>
            <Select
              labelId="leaseStatusLabel"
              id="leaseStatus"
            ...
            >
              <MenuItem value={true}>Leased</MenuItem>
              <MenuItem value={false}>Unleased</MenuItem>
            </Select>
          </FormControl>
          ...
          <Button
            ...
            variant="contained"
            color="primary"
            id="ownerAddPropertyButton"
            className={classes.saveButton}
            onClick={(event) => { event.preventDefault(); saveNewProperty(imageUpload, Address, postcode, leaseStatus); }}
          >
            Save
          </Button>
          <Box pt={4}>
...
async function saveNewProperty (imageUpload, Address, postcode, leaseStatus) {
  const formData = new FormData();
  formData.append('address', Address);
  formData.append('post_code', postcode);
  formData.append('leased', leaseStatus);
  ...
  if (res.status === 200) {
    console.log('New property saved success');
    sessionStorage.setItem('token', data.token);
    window.location = '/owner/properties';
  } else if (res.status === 400 || res.status === 403) {
    console.log('New property saved fail');
    console.log(data.error);
    alert(data.error);
    // window.location = '/';
  } else {
    console.log('New property saved fail');
    alert('New property saved fail');
  }
  sessionStorage.setItem('token', data.token);
}
