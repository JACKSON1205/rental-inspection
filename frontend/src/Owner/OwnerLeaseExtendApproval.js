...
export default function NotificationDetail (props) {
  ...
  React.useEffect(() => {
    ...
    async function getArtifact () {
      ...
      if (res.status === 200) {
        setExtenstionDate(data.artifact_resp.artifact_json.extension_date);
        sessionStorage.setItem('token', data.token);
      ...
      <main className={classes.content}>
        ...
        <Container maxWidth="lg" className={classes.container}>
          ...
          <Typography component="h1" variant="h4" align="center">
            Do you approve this extension of lease ?
          </Typography>
          <br />
          <br />
          ...
          <div className={classes.buttonsContainer}>
            <Button
              ...
              onClick={(event) => { event.preventDefault(); approveRequest(artifactId[notificationIndex]); }}
            >
              Approve
            </Button>
            <Button
              ...
              onClick={(event) => { event.preventDefault(); declineRequest(artifactId[notificationIndex]); }}
            >
              Decline
            </Button>
          </div>
...
// Make api request of accepting request from manager
async function approveRequest (artifactId) {
  const options = {
    method: 'PATCH',
    headers: {
      accept: 'application/json',
  ...
  if (res.status === 200 || res.status === 201) {
    console.log('Approve the extension request');
    sessionStorage.setItem('token', data.token);
...
// Make api request of denieding request from manager
async function declineRequest (artifactId) {
  const options = {
    method: 'PATCH',
    headers: {
      accept: 'application/json',
