  ...
  return (
    <Grid item xs={7}>
      <Paper className={fixedHeightPaper}>
        ...
        {notification.text.startsWith('Welcome') ? <IconButton onClick={(event) => { event.preventDefault(); window.location = '/owner/contact'; }}><OpenInNewIcon /></IconButton> : null}
        ...
        {notification.artifact_type === 'request_repair' ? <IconButton onClick={(event) => { event.preventDefault(); openNotification(notification.notification_id, notification.artifact_type, notification.artifact_id); }}><OpenInNewIcon /></IconButton> : null}
        {notification.artifact_type === 'notice_leave' ? <IconButton onClick={(event) => { event.preventDefault(); openNotification(notification.notification_id, notification.artifact_type, notification.artifact_id); }}><OpenInNewIcon /></IconButton> : null}
        {notification.artifact_type === 'request_lease_extension' ? <IconButton onClick={(event) => { event.preventDefault(); openNotification(notification.notification_id, notification.artifact_type, notification.artifact_id); }}><OpenInNewIcon /></IconButton> : null}
...
async function openNotification (notificationsId, artifactType, artifactId) {
  ...
  if (artifactType === 'connection_from_manager' && data.artifact_resp.status === 'pending') {
    sessionStorage.setItem('token', data.token);
    window.location = '/owner/connection_approval/' + notificationsId;
  } else if (artifactType === 'request_repair' && data.artifact_resp.status === 'pending') {
    sessionStorage.setItem('token', data.token);
    window.location = '/owner/maintenance_approval/' + notificationsId;
  } else if (artifactType === 'notice_leave' && data.artifact_resp.status === 'pending') {
    sessionStorage.setItem('token', data.token);
    window.location = '/owner/notice_leave/' + notificationsId;
  } else if (artifactType === 'request_lease_extension' && data.artifact_resp.status === 'pending') {
    sessionStorage.setItem('token', data.token);
    window.location = '/owner/lease_extension/' + notificationsId;
  } else if (artifactType === 'notice' && data.artifact_resp.status === 'pending') {
    sessionStorage.setItem('token', data.token);
    window.location = '/owner/notice_detail/' + notificationsId;
