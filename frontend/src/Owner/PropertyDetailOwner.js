            ...
            <Paper className={classes.paper}>
              <Grid container spacing={3}>
                ...
                <Grid item xs={6}>
                  ...
                  <br />
                  <br />
                  {propertyDetail.leased !== true
                    ? <Button
                      variant="contained"
                      color="secondary"
                      id="requestUnlistingButton"
                      className={classes.requestUnlistingButton}
                      onClick={(event) => { event.preventDefault(); requestUnlistingProperty(propertyId); }}
                    >
                      Request Unlisting
                    </Button>
                    : null}
                  ...
                  <Button
                    ...
                    className={classes.updateImageButton}
                    onClick={(event) => { event.preventDefault(); updateOwnerImage(imageUpload, propertyId); }}
                  >
                    Save
                  </Button>
                </Grid>
            ...
            <Grid item xs={5}>
              <Paper className={fixedHeightPaper}>
                <Typography component="body2" variant="inherit" color="inherit" align="left" noWrap className={classes.smallContext}>
                  Manager: {propertyDetail.manager_first_name} {propertyDetail.manager_last_name}
                </Typography>
                <br />
                <Typography component="body2" variant="inherit" color="inherit" align="left" noWrap className={classes.smallContext}>
                  Manager Email: {propertyDetail.manager_email}
                </Typography>
                <br />
...
async function updateOwnerImage (imageUpload, propertyId) {
...
// Make api request of listing owner property
async function requestListingProperty (propertyId) {
  const artifactJSON = {
  ...
  if (res.status === 200 || res.status === 201) {
    console.log('Request listing property success');
    sessionStorage.setItem('token', data.token);
    alert('Request listing property success, wait for the manager approval');
    window.location = '/owner/property_detail/' + propertyId;
  } else if (res.status === 400 || res.status === 403) {
    console.log('Request listing property fail');
    console.log(data.error);
    alert(res.status + ': ' + data.error);
  } else {
    console.log('Request listing property fail');
    alert('Request listing property fail');
  }
  sessionStorage.setItem('token', data.token);
}

// Make api request of unlisting owner property
async function requestUnlistingProperty (propertyId) {
  const artifactJSON = {
    property_id: propertyId,
  };
  const options = {
    ...
    body: JSON.stringify({
      artifact_json: artifactJSON,
      artifact_type: 'request_unlisting',
    }),
  }
  // Get data using fetch
  console.log(options);
  ...
  if (res.status === 200 || res.status === 201) {
    console.log('Request unlisting property success');
    sessionStorage.setItem('token', data.token);
    alert('Request unlisting property success, wait for the manager approval');
    window.location = '/owner/property_detail/' + propertyId;
  } else if (res.status === 400 || res.status === 403) {
    console.log('Request unlisting property fail');
    console.log(data.error);
    alert(res.status + ': ' + data.error);
    // window.location = '/';
  } else {
    console.log('Request unlisting property fail');
    alert('Request unlisting property fail');
  }
  sessionStorage.setItem('token', data.token);
}
